...
public class ModEvents {
    ...
    public static void playerJoinEvent(PlayerEntity player) {
        ...
        if (ModConfigs.devMode) {
            player.sendMessage(new LiteralText("==================== VMTU Dev Mode ===================="));
        ...
        if (!translation.getLanguage().equals(languageManager.getLanguage().getCode()) && LanguageHelper.isChineseLanguage()) {
            player.sendMessage(new TranslatableText("vmtranslationupdate.message.language_not_support", translation.getLanguage()));
        }

        if (ModConfigs.checkModPackTranslationUpdate) {
            if (!onlineVersion.isValid()) {
                player.sendMessage(new TranslatableText("vmtranslationupdate.message.update.error"));
            ...
            if (translationUpdateNeeded) {
                player.sendMessage(new TranslatableText("vmtranslationupdate.message.update.new_version.text_part1", coloredLocalVer.getFormattedString(), coloredOnlineVer.getFormattedString()));
                String updateUrl = translation.getUrl();
                Text message = new TranslatableText("vmtranslationupdate.message.update.new_version.text_part2")
                        .append(new TranslatableText(updateUrl)
                                .setStyle(new Style()
                                        .setClickEvent(GameEventHelper.clickOpenUrl(updateUrl))
                                        .setHoverEvent(GameEventHelper.hoverShowText(new TranslatableText("vmtranslationupdate.message.update.new_version.download_hover")))
                                        .setColor(Formatting.AQUA)
                                ))
                        .append(new TranslatableText("vmtranslationupdate.message.update.new_version.text_part3"));
                ...
                if (modpackUpdateNeeded){
                    ...
                    player.sendMessage(new TranslatableText("vmtranslationupdate.message.update.modpack_version_error"));
                    player.sendMessage(new TranslatableText("vmtranslationupdate.message.update.modpack_version_error.hint", coloredLocalModpackVer.getFormattedString(), coloredOnlineModpackVer.getFormattedString()));
                }
            }
        }
    }
