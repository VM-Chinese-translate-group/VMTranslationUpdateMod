...
public class ScreenHelper extends DrawableHelper {
    ...
    public static void drawGuiTexture(MatrixStack matrixStack, int startX, int startY, int endX, int endY, int colorStart, int colorEnd, int z) {
        DrawableHelper.fillGradient(matrixStack, startX, startY, endX, endY, colorStart, colorEnd, z);
    }
    ...
    public static void openUrlOnScreen(MinecraftClient client, Screen screen, String url) {
        if (StringUtils.isNotBlank(url) && client != null) {
            client.setScreen(new ConfirmChatLinkScreen(yes -> {
                if (yes) {
